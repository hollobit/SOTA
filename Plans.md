...
## Current Status: Session 128 — 11-URL targeted (Kimi K2.7 Code + GLM-5.2) (2026-06-16)
**3,094 models · 3,234 benchmarks · 15,097 scores · 🌍 +1 model / 6 benches / +6 scores**

### 2026-06-16 Session 128 — 11-URL targeted mining (Kimi K2.7 Code + GLM-5.2)
- Single agent on 11 URLs 🌍 **+1/6/+6**. **moonshot/kimi-k2.7-code architecture** (Moonshot launch page): 1T total / 32B active MoE + MLA attention + **MoonViT 400M vision encoder** + 262K ctx + **text+image+video**. Pricing: Kimi API $0.19/$0.95 in (cache hit/miss) + $4.00 out per 1M. OpenRouter $0.75/$3.50. **Open weights Modified MIT**. **K2.7 Code per-bench breakdown** (Moonshot self-report): Kimi Code Bench v2 62.0 / Program Bench 53.6 / MLS Bench Lite 35.1 / Kimi Claw 24/7 Bench 46.9 / MCP Atlas 76.0 / MCP Mark Verified 81.1. **Comparators positioning** (already in DB): GPT-5.5 leads Kimi Code v2 69.0 + Program 69.1 + MCP Mark V 92.9. Opus 4.8 leads MLS Bench Lite 42.8. Gemini 3.5 Flash MCP Atlas 83.6 SOTA. **K2.7 Code positions between Opus 4.8 and GPT-5.5 on all 6 benches** — Moonshot pitches price/performance + open-weight leader, not capability SOTA. **11-URL gaps**: 8/11 URLs blocked WebFetch (BridgeBench SpeedBench/Reasoning x2, Reddit ZaiGLM, DigitalApplied, CoderSera x2, Flowtivity) or timed out (llm-stats). **GLM-5.2 has ZERO attributable benchmark scores from this pass** — needs Playwright fallback for bridgebench.ai + llm-stats
